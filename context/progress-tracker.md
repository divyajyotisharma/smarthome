...
- Appliance registration idempotency uses `home_id + vendor + vendor_device_id`; new registrations return `201 Created`, and idempotent replays return `200 OK` with the existing appliance.
- Appliance registration rejects non-positive `collection_interval_seconds` values.
...
- API edge coverage verification: added tests for missing home lookup, missing registration fields, non-positive collection intervals, home/vendor-scoped idempotency, inactive duplicate registration, cross-home appliance access, deactivation metrics without prior readings, metrics visibility for deactivation events, and report summaries for deactivation events. This found and fixed one validation gap: `collection_interval_seconds = 0` was accepted before schema validation was tightened. Final regression `.venv/bin/python -m pytest` passed with 53 tests and 1 FastAPI/Starlette TestClient deprecation warning.
- API status-code review: corrected idempotent duplicate appliance registration from `201 Created` to `200 OK` because no new resource is created; new registrations still return `201 Created`. Final regression `.venv/bin/python -m pytest` passed with 54 tests and 1 FastAPI/Starlette TestClient deprecation warning.
